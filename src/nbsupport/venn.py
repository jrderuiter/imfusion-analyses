...
#--------------------------------------------------------------------
def venn(data, names=None, fill="number", show_names=True, **kwds):
    """
    data: a list
    ...
    if len(data) == 2:
        return venn2(data, names, fill, show_names, **kwds)
    elif len(data) == 3:
        return venn3(data, names, fill, show_names, **kwds)
    elif len(data) == 4:
        return venn4(data, names, fill, show_names, **kwds)
    else:
...
def venn2(data=None, names=None, fill="number", show_names=True, **kwds):
    ...
    if 'colors' in kwds and isinstance(kwds['colors'],
                                       Iterable) and len(kwds['colors']) >= 2:
        colors = kwds['colors']
    else:
        colors = ['red', 'green']
    ...
    if show_names:
        plt.text(x1, y1 - 1.2 * r, names[0], **alignment)
        plt.text(x2, y2 - 1.2 * r, names[1], **alignment)
...
def venn3(data=None, names=None, fill="number", show_names=True, **kwds):
    ...
    # set colors for different Circles or ellipses
    if 'colors' in kwds and isinstance(kwds['colors'],
                                       Iterable) and len(kwds['colors']) >= 3:
        colors = kwds['colors']
    else:
    ...
    # names of different groups
    if show_names:
        plt.text(x1 - r, y1 - r, names[0], **alignment)
        plt.text(x2 + r, y2 - r, names[1], **alignment)
        plt.text(x3, y3 + 1.2 * r, names[2], **alignment)
...
def venn4(data=None, names=None, fill="number", show_names=True, **kwds):
    ...
    patches.append(
        Ellipse(
            (170, 170), width, height, -45, color=colors[0], alpha=0.5))
    patches.append(
        Ellipse(
            (200, 200), width, height, -45, color=colors[1], alpha=0.5))
    patches.append(
        Ellipse(
            (200, 200), width, height, -135, color=colors[2], alpha=0.5))
    patches.append(
        Ellipse(
            (230, 170), width, height, -135, color=colors[3], alpha=0.5))
    ...
    # names of different groups
    if show_names:
        plt.text(110, 110, names[0], **alignment)
        plt.text(290, 110, names[1], **alignment)
        plt.text(130, 275, names[2], **alignment)
        plt.text(270, 275, names[3], **alignment)
